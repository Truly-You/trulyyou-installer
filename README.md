...
Choose your provider above, open its cloud shell and run the displayed installer command. Use a dedicated account, subscription or project with billing enabled and an existing public DNS zone. The installer asks for a new dashboard hostname in that zone plus your company and owner email. It creates HTTPS ingress, a dedicated dashboard VM, private versioned object storage and a deployment identity. The dashboard then provisions the login backend and its datastore and gateway.

Supported regions are AWS `eu-west-1` and `us-east-1`, Azure `westeurope` and `eastus`, and Google Cloud `europe-west1` and `us-central1`. These installers still need live-account validation. Their local storage and bootstrap tests pass; this does not establish that a fresh cloud deployment succeeds.
